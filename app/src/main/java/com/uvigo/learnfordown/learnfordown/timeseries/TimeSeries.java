...
package com.uvigo.learnfordown.learnfordown.timeseries;
...
import com.uvigo.learnfordown.learnfordown.util.Arrays;
...
public class TimeSeries {
		...
		try {
			...
			if (isLabeled) {
				...
				if (labels.size() == 0) {
					...
					throw new InternalError("ERROR:  The time column (1st col) in a time series must be labeled as 'Time', '" + labels.get(0)
							+ "' was found instead");
				}
			...
			while ((line = br.readLine()) != null) {
				if (line.length() > 0) // ignore empty lines
				{
					...
					final TimeSeriesPoint readings = new TimeSeriesPoint(currentLineValues.subList(firstMeasurement, currentLineValues.size()));
	...
	public TimeSeries(List<Float> inputUnlabeledSingleColumnData) {
		labels = new ArrayList(2);
		timeReadings = new ArrayList(inputUnlabeledSingleColumnData.size());
		tsArray = new ArrayList(inputUnlabeledSingleColumnData.size());

		labels.add("Time");
		labels.add("c1");

		for (float data : inputUnlabeledSingleColumnData) {
			timeReadings.add(new Double(timeReadings.size()));
			final double[] tsPoint = { data };
			tsArray.add(new TimeSeriesPoint(tsPoint));
		}
	}

	public TimeSeries(ArrayList<LinkedList<Float>> inputUnlabeledMultiColumnData) {
		this();

		labels.add("Time");

		ArrayList<Iterator<Float>> inputInterators = new ArrayList<Iterator<Float>>(inputUnlabeledMultiColumnData.size());
		for (int c = 0; c < inputUnlabeledMultiColumnData.size(); c++) {
			labels.add("c" + (c + 1));
			inputInterators.add(c, inputUnlabeledMultiColumnData.get(c).iterator());
		}

		while (inputInterators.get(0).hasNext()) {
			timeReadings.add(new Double(timeReadings.size()));

			final double[] tsPoint = new double[inputUnlabeledMultiColumnData.size()];
			for (int c = 0; c < inputUnlabeledMultiColumnData.size(); c++) {
				tsPoint[c] = inputInterators.get(c).next();
			}
			tsArray.add(new TimeSeriesPoint(tsPoint));
		}
	}

	public TimeSeries(LinkedList<Point2D> normalizedPoints) {
		labels = new ArrayList(3);
		timeReadings = new ArrayList(normalizedPoints.size());
		tsArray = new ArrayList(normalizedPoints.size());
		...
		for (Point2D point : normalizedPoints) {
			timeReadings.add(new Double(timeReadings.size()));
			final double[] tsPoint = new double[] { point.x, point.y };
			tsArray.add(new TimeSeriesPoint(tsPoint));
		}
	}
	...
	public void addFirst(double time, TimeSeriesPoint values) {
		if (labels.size() != values.size() + 1) {
			// time
			throw new InternalError("ERROR:  The TimeSeriesPoint: " + values + " contains the wrong number of values. " + "expected:  " + labels.size() + ", "
					+ "found: " + values.size());
		}

		if (time >= ((Double) timeReadings.get(0)).doubleValue()) {
			throw new InternalError("ERROR:  The point being inserted into the " + "beginning of the time series does not have "
	...
	public String toString() {
		...
		/*
		 * // Write labels for (int x=0; x<labels.size(); x++) { outStr.append(labels.get(x)); if (x < labels.size()-1) outStr.append(","); else
		 * outStr.append("\n"); } // end for loop
		 */
	...
	private static boolean determineIsFirstColTime(String filePath) {
		...
		try {
			...
			final double expectedDiff = ((Double) possibleTimeValues.get(1)).doubleValue() - ((Double) possibleTimeValues.get(0)).doubleValue();
			final double flexibility = expectedDiff * EQUALITY_FLEXIBILITY_PCT;
			for (int x = 1; x < possibleTimeValues.size(); x++) {
				if (Math.abs(((Double) possibleTimeValues.get(x)).doubleValue() - ((Double) possibleTimeValues.get(x - 1)).doubleValue() - expectedDiff) > Math
						.abs(flexibility)) {
					return false;
				}
			}
